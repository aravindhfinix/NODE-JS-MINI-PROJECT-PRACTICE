 ...
 * @returns {Promise}
 */
Babbel.prototype.translate = function(text, target, source, apiKey) {
  return Promise.resolve()
    .then(() => {
      if (!is.string(text) && !(is.array(text) && text.length && is.all.string(text))) {
      ...
      return this.client(request)
        ...
        .catch(err => {
          if (err.response) {
            ...
            throw new this.RequestError(`[${code}] ${message} [${request.params.key}]`);
          } else {
            throw err;
          }
        });
